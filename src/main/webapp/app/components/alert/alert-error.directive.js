(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('hipsterHousesApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-hipsterHousesApp-error');
                var entityKey = httpResponse.headers('X-hipsterHousesApp-params');
                if (errorHeader) {
